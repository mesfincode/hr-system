...
import { columns } from "../table-components/columns";
import { getClientWithPagination, getFilteredClient } from "@/actions/clientAction";

const ClientView = () => {
 
  const router = useRouter();
  const [isMounted, setIsMounted] = useState(false);
  const [employeeList, setEmployeeList] = useState<Client[]>([]);  
 ...
 const getEmploye =async()=>{
  ...
  const employeeData = await getClientWithPagination(pagenationOption)
...
const getFilteredEmp = async(searchCriteria: string ,
  ...
  let pagenationOption = {page:page,pageSize:pageSize}
  const employeeData = await getFilteredClient(searchCriteria,page,pageSize)
  console.log(employeeData)
  setEmployeeList(employeeData.data)
  setPage(employeeData.page)
  // setPageSize(employeeData.pageSize)
